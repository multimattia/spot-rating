import { l as lucia } from './pages/_id__CN_NXpWk.mjs';
import 'lucia';
import { d as db, U as User } from './pages/_id__ChIl-fI5.mjs';
import { sql } from '@astrojs/db/dist/runtime/virtual.js';
import { A as AstroError, Z as ResponseSentError } from './astro_Ccjqjgku.mjs';
...
const onRequest$1 = defineMiddleware(async (context, next) => {
  ...
  let existingUser;
  try {
    existingUser = (await db.select().from(User).where(sql`${User.id} = ${session ? session.userId : ""}`))[0];
  } catch (e) {
    console.error(e);
  }
